...
#define  SHL(x,n) (((x) & 0xFFFFFFFF) << ((unsigned int)n % 32))
#define  ROTL(x,n) (SHL((x),n) | ((x) >> ((unsigned int)(32 - n) % 32)))
...
void sm3_finish( sm3_context *ctx, unsigned char output[32] )
{
    ...
    PUT_Uint_BE( ctx->state[5], output, 20 );
    PUT_Uint_BE( ctx->state[6], output, 24 );
    PUT_Uint_BE( ctx->state[7], output, 28 );
}

/*
 * output = SM3( input buffer )
 */
void opcsm3( unsigned char *input, int ilen,
